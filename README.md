...
>
> 🩹 **v3 修正（2026-09-22）**：mihomo 侧文件名 `.list` → **`.yaml`**，内容改为顶层 `payload` 列表。`.list` 是 Surge / Quantumult 的惯例；mihomo 的 `format` **默认就是 `yaml`**，沿用 `.list` 会让抄配置的人要么漏写 `format`（按 yaml 解析直接报错），要么被迫写一条多余的 `format: text`。**旧地址已失效，请按下方配置换 URL。** Surge 侧文件名不变。
...
```
1️⃣ 选文件   →   mihomo-ads.yaml（mihomo）/ surge-ads.list（Surge）
...
| 客户端 | 黑名单 | 白名单 |
|:-------|:-------|:-------|
| mihomo / OpenClash / Stash | `mihomo-ads.yaml` | `mihomo-white-guard.yaml` |
...
```
jinx-ads-rules/
├── mihomo-ads.yaml             # 黑名单 完整版 3891 条 ⭐
├── mihomo-ads-delta.yaml       # 黑名单 差集版 3838 条
├── mihomo-white-guard.yaml     # 白名单守卫 43 条 ⭐
...
> 📐 **后缀即格式**：`mihomo-*.yaml` 是 mihomo 的 `payload` 列表（配 `format: yaml`）；`surge-*.list` 是 Surge RULE-SET 文本。两者内容**不可互换**。
...
>
> `format` 写 `yaml`（同时也是默认值）—— 文件内容是顶层 `payload` 列表。写成 `text` 会解析失败。
>
...
| `skill/scripts/upload_repo.py` | 按「本地:仓库路径」映射上传，自带回拉 md5 校验 |
| `skill/scripts/upload_to_github.py` | 建仓 + 批量上传辅助脚本（纯 GitHub API，无需 git / gh CLI） |
...
> ✅ **已实测**：从上游 `3.1.9` 重跑 —— Surge 三个文件与仓库现有文件**逐字节一致**（md5 未变）；mihomo 三个 `.yaml` 的 `payload` 与旧 `.list` 的规则行**逐条等价**（3891 / 3838 / 43），且能被标准 YAML 解析器解析。
...
两者都追加在输出**末尾**、表头多一行 `# extra:` / `# extra-white:`，便于 diff 核验。

- ⚠️ **不要手改 `mihomo-*.yaml` / `surge-*.list`** —— 它们是生成产物，重跑一次即被完全覆盖。
